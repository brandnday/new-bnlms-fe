...
import {
  GET_AVAILABLE_CHURCH_ENDPOINT,
  AUTHORIZE_ENDPOINT
} from "../../constant/apiEndpoints";
import { selectToken, selectSelectedChurch } from "./AccountReducer";
export const getAvailableChurchList = nickname => async (
  dispatch,
  getState
) => {
  const res = await dispatch(
    callPostWithAuth(GET_AVAILABLE_CHURCH_ENDPOINT, {
...
export const checkAuthorize = (token, role) => async (dispatch, getState) => {
  const res = await dispatch(
    callPostWithAuth(AUTHORIZE_ENDPOINT, {
      role
    })
  );
  dispatch(updateRole(res.role));
  return res;
};

export const updateRole = role => ({
  type: AccountActionTypes.CURRENT_ROLE_UPDATE,
  payload: role
});
